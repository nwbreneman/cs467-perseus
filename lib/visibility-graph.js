var VisGraph = function () {
    ...
    this.addNode = function (node) {
        if (node.x && node.y) {
            if (this.G.findIndex(function (e) { return e.x === node.x && e.y === node.y }) === -1) {
                this.G.push(node);
            }
        }
    }
    ...
    this.removeNode = function (node) {
        var index = this.G.findIndex(function (e) { return e.x === node.x && e.y === node.y });
        if (index > -1) {
            this.G.splice(index, 1);
        }
    }

    this.addVertex = function (vertex, self) {
        if (vertex.x && vertex.y) {
            vertex.id = this.G.length;
            for (var i = 0; i < this.G.length; i++) {
                v = this.G[i];
                if (isVisible(v.x, v.y, vertex.x, vertex.y, self)) {
...
Vertex.prototype.addNeighbor = function (v) {
    if (this.neighbors.findIndex(function (e) { return e.x === v.x && e.y === v.y }) === -1) {
        this.neighbors.push(v);
    }
};


var isVisible = function (x1, y1, x2, y2, self) {
    ...
    if (result.length > 0) {
        if (self) {
            var selfIndex = result.indexOf(self);
            if (selfIndex > -1) {
