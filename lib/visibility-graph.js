var VisGraph = function () {
    ...
    this.addNode = function (node) {
        if (this.G.map(function (e) { return e.id; }).indexOf(node.id) === -1) {
            this.G.push(node);
        }
    }

    this.removeNode = function (node) {
        var index = this.G.map(function (e) { return e.id; }).indexOf(node.id);
        if (index > -1) {
...
Vertex.prototype.addNeighbor = function (v) {
    if (this.neighbors.map(function (e) { return e.id; }).indexOf(v.id) === -1) {
    ...
    for (var i = 1; i < cols; i++) {
        for (var j = 1; j < rows; j++) {
            ...
            if (layer.getTile(coords.x, coords.y)) {
                v = new Vertex(coords.x, coords.y, idCount);
                v.TILE = true;
                vertices.push(v);
                idCount++;
            }
